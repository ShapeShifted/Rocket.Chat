...
import { Box, Button, ButtonGroup, Select } from '@rocket.chat/fuselage';
import { PageHeader } from '/client/components/Page';
import { AnnouncementService, CONFIG_FILES, ConfigOption } from './services/announcement.service';

const Announcement = (): ReactElement => {
  const [selectedKey, setSelectedKey] = useState<string>(CONFIG_FILES[0].key);
  ...
  const activeConfig = CONFIG_FILES.find((c) => c.key === selectedKey) || CONFIG_FILES[0];
  ...
  useEffect(() => {
    const loadConfig = async () => {
      ...
      setSuccess(false);
      try {
        const config = await AnnouncementService.getWelcomeConfig(activeConfig.type, activeConfig.lang);
        setMessage(config.message || '');
        setOptionsText((config.options || []).join('\n'));
      } catch (err: any) {
        setError(err.message || `Failed to load configuration for ${activeConfig.fileName}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err.message || `Failed to save configuration for ${activeConfig.fileName}`);
    } finally {
      setSaving(false);
    }
  };

  const selectOptions: [string, string][] = CONFIG_FILES.map((cfg) => [
    cfg.key,
    `${cfg.label} (${cfg.fileName})`,
  ]);

  return (
    <Box style={{ maxWidth: 800, width: '100%', margin: '0 auto', padding: '0 24px' }}>
      <PageHeader title="Welcome Message & Announcement Configurator" />
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <Select
            ...
            placeholder="Select a JSON configuration file..."
          />
          <span style={{ fontSize: '12px', color: '#6c757d', marginTop: 4 }}>
            Currently editing file: <strong>{activeConfig.fileName}</strong> ({activeConfig.label})
          </span>
        </div>
